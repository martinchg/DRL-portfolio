...
**Generative scenario engine — Phase 1, a documented negative (Act 4)** — a from-scratch **DDPM on return windows** (1D U-Net, cosine schedule, EMA), judged by a **pre-registered validation protocol**: acceptance bands calibrated on real data, three baselines (i.i.d. Gaussian, i.i.d. bootstrap, per-ticker GARCH(1,1)-t), a frozen GRU discriminative judge with purged/embargoed splits, and six GO/NO-GO criteria frozen *before* any training. After the pre-registered two-iteration budget (ε-prediction, then v-prediction): the model **captures volatility clustering** (ACF|r| lag-1 = 0.143, inside the real band) and does not memorize the train set, but **under-disperses the multi-asset scale mixture** (generated σ ×0.56, high-vol tail ×0.43) and fails the discriminative bar (acc 0.868 vs threshold 0.682) → **NO-GO: no RL training on synthetic data**. The sampler was exonerated by an analytical oracle test (closed-form optimal denoiser → std 0.9985, now a permanent unit test). Full post-mortem in the report (Acte 4).

Known limitations: regime-dependent performance (see walk-forward above), alpha mostly carried by the stop rule, Max Drawdown bounded by the kill-switch (~25%), absolute Sharpe below 1. Reproduce with `python train.py`, `python evaluate.py`, `python walk_forward.py`, `python seed_robustness.py`, `python regime_experiment.py`; diffusion: `python validate_diffusion.py --calibration-only`, `python train_diffusion.py`, `python validate_diffusion.py --model-dir models/diffusion_v2`; run the test suite with `python -m pytest` (103 tests).
