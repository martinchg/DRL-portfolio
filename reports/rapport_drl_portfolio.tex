...
\code{tests/} & Qualité & 67 tests pytest : pipeline de données, mécanique de trading,
...
\begin{enumerate}[itemsep=2pt]
  \item \textbf{features de régime} (tendance longue, distance au plus-haut d'un an,
        ...
        Markov-switching, vus en économétrie) ;
...
\subsection{Les questions auxquelles je n'ai pas encore de mesure}

\begin{itemize}[itemsep=2pt]
  \item \emph{Sensibilité au seed d'entraînement} : tous mes modèles partagent le seed
        42 ; il faudrait ré-entraîner sur plusieurs seeds pour donner un intervalle sur
        l'alpha (coût : quelques heures de calcul).
...
\begin{itemize}[itemsep=3pt]
  \item \textbf{Profil régime-dépendant} (section~\ref{sec:wf}) : la limite principale
        désormais, avec ses trois pistes de réponse (features de régime, position
...
\begin{description}[itemsep=4pt, font=\normalfont\bfseries]
  ...
  \item[Overfitting / underfitting] Apprendre par c\oe ur le train (excellent
        ...
        mémorisation ; mais mon vrai plafond est un \emph{underfitting de
        représentation} : aucune capacité de réseau ne compense des features qui ne
        contiennent pas l'information de régime.
  ...
  \item[Seed] Graine du générateur pseudo-aléatoire. La fixer rend l'expérience
        ...
        la faire \emph{varier} mesure la sensibilité au hasard --- ce que je n'ai pas
        encore fait pour l'entraînement, et que je liste honnêtement dans les
        questions ouvertes.
...
\begin{itemize}[itemsep=1pt]
  ...
  \item Suite de tests : \code{python -m pytest} (67 tests ; marqueurs \code{network}
...
\begin{itemize}[itemsep=2pt]
  ...
  \item le terme de \textbf{valeur} entraîne le critique ($V_t^{\text{cible}}$ est le
        rendement observé bootstrappé) ; sans bon critique, pas de bon avantage ;
